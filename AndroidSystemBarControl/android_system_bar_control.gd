...
func toggle_system_bars_transparency(transparent: bool, dark: bool = false) -> void:
	if transparent:
		activity.runOnUiThread(android_runtime.createRunnableFromGodotCallable(make_system_bars_transparent))
	else:
		activity.runOnUiThread(android_runtime.createRunnableFromGodotCallable(make_system_bars_opaque))
